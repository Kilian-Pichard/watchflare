...
func printHelp() {
	...
	fmt.Println("  install    --token=TOKEN [--host=HOST] [--port=PORT]  Install and register the agent")
	fmt.Println("  uninstall                                              Remove the agent")
	fmt.Println("  register   --token=TOKEN [--host=HOST] [--port=PORT]  Re-register with a new backend")
	fmt.Println()
