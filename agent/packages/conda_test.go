...
func TestParseCondaLine(t *testing.T) {
	tests := []struct {
		input       string
		wantNil     bool
		name        string
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			...
			if pkg.PackageManager != "conda" {
				t.Errorf("package manager: got %q, want %q", pkg.PackageManager, "conda")
			}
