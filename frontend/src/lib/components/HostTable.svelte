<script lang="ts">
    ...
    import type { HostWithMetrics, Metric, Host } from "$lib/types";
        ...
        const sorted = [...filtered].sort((a, b) => {
            ...
            switch (sortColumn) {
                ...
                case "status":
                    ...
                    break;
                case "cpu": {
                    const mA = getLastMetrics(a.host.id);
                    const mB = getLastMetrics(b.host.id);
                    valA = mA.cpu;
                    valB = mB.cpu;
                    break;
                }
            ...
            >
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2 min-w-0">
                        <span
                            class="h-2 w-2 shrink-0 rounded-full {host.status ===
                            'online'
                                ? 'bg-success'
                                : host.status === 'offline'
                                  ? 'bg-danger'
