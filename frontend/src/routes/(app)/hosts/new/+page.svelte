<script lang="ts">
	...
	async function handleSubmit(e) {
		...
		try {
			const response = await api.createHost(name, configuredIP, allowAnyIP);
			success = true;
			createdHost = response.host;
			token = response.token;
			agentKey = response.agent_key;
			backendHost = window.location.hostname;
		} catch (err) {
			error = err.message || 'Failed to create host';
		} finally {
			loading = false;
		}
	}
...
<svelte:head>
	<title>Add Host - Watchflare</title>
</svelte:head>
...
{#if !success}
	...
	<div class="max-w-2xl">
		<div class="mb-6">
			<h1 class="text-2xl font-semibold text-foreground">Add New Host</h1>
			<p class="text-sm text-muted-foreground mt-1">Configure a new host to monitor</p>
		</div>
			...
			<form onsubmit={handleSubmit}>
				...
				<div class="mb-4">
					<label for="name" class="block text-sm font-medium text-foreground mb-2">
						Host Name <span class="text-destructive">*</span>
					</label>
					<input
						id="name"
						type="text"
						bind:value={name}
						placeholder="e.g., web-host-01"
				...
				<div class="mb-4">
					...
					<p class="mt-1 text-xs text-muted-foreground">
						The IP address you expect this host to connect from
					</p>
				</div>
					...
					<label class="flex items-start gap-2 cursor-pointer">
						...
						<div>
							...
							<p class="text-xs text-muted-foreground mt-0.5">
								If enabled, the host can register from any IP address
							</p>
						</div>
					</label>
				</div>
...
{:else}
	...
	<div class="max-w-3xl">
		...
		<InstallInstructions host={createdHost} {token} {agentKey} {backendHost} />

		<!-- Actions -->
		<div class="mt-6 flex gap-3">
			<button
				onclick={() => goto(`/hosts/${createdHost.id}`)}
