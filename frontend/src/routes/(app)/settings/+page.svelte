...
{#if !cookieSecure}
    <div role="alert" class="mb-6 flex items-start gap-3 rounded-lg border border-warning/40 bg-warning/10 px-4 py-3 text-sm text-warning">
        ...
        <div>
            ...
            <p class="mt-0.5 text-warning/80">Serve Watchflare over HTTPS to enable secure cookies. Either configure direct TLS via <code class="font-mono text-xs bg-warning/10 px-1 rounded">TLS_CERT_FILE</code> / <code class="font-mono text-xs bg-warning/10 px-1 rounded">TLS_KEY_FILE</code>, or use a reverse proxy (Nginx, Caddy, Traefik) and add its IP to the <code class="font-mono text-xs bg-warning/10 px-1 rounded">TRUSTED_PROXIES</code> environment variable.</p>
        </div>
    </div>
{/if}
